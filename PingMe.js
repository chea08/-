...
function notify(title, body) {
  if (typeof $notification !== 'undefined') $notification.post(scriptName, title, body);
  else if (typeof $notify !== 'undefined') $notify(scriptName, title, body);
  ...
  return new Promise((resolve, reject) => {
    if (typeof $httpClient !== 'undefined') {
      $httpClient.get({ url, headers }, (err, resp, data) => {
        if (err) reject(err);
        else resolve({ status: resp && resp.status, body: data });
      });
    } else if (typeof $task !== 'undefined') {
      $task.fetch({ url, method: 'GET', headers }).then(resolve).catch(reject);
    } else {
      reject('No http client available');
    }
  });
}

function done(val) { $done(val || {}); }
...
function runCapture() {
  ...
  writeStore(JSON.stringify(capture), ckKey);
  notify('✅ 参数抓取成功', '已保存请求头+参数');
  done({});
}

function runTask() {
  const raw = readStore(ckKey);
  if (!raw) {
  ...
  function doVideoLoop(count) {
    ...
    for (let i = 1; i <= count; i++) {
      p = p.then(() => new Promise(resolve => setTimeout(resolve, i === 1 ? 1500 : VIDEO_DELAY)))
        .then(() => callApi('videoBonus'))
        .then(res => {
          try {
            const d = JSON.parse(res.body);
            if (d.retcode === 0) msgs.push(`🎬 视频${i}：+${(d.result && d.result.bonus) || '?'} Coins`);
        ...
        .catch(err => {
          msgs.push(`❌ 视频${i}：${String(err)}`);
        });
    }
    return p;
  }

  callApi('queryBalanceAndBonus')
    .then(res => {
      try {
        const d = JSON.parse(res.body);
    ...
    .then(() => callApi('queryBalanceAndBonus'))
    .then(res => {
      try {
        const d = JSON.parse(res.body);
...
if (typeof $request !== 'undefined' && $request) runCapture();
else runTask();